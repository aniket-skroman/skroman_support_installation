...
func (ser *complaint_service) Fetch_user_info(user_id, user_type, dept string) (dto.AllocatedEmpDetailsDTO, error) {
	...
	api_request := proxycalls.NewAPIRequest("fetch-user", http.MethodGet, false, nil, nil, map[string]string{
		"Authorization": token,
	})
	...
	if response.StatusCode == http.StatusOK {
		...
		return dto.AllocatedEmpDetailsDTO{
			AllocateUserID: response_data.UserData.ID,
			FullName:       response_data.UserData.FullName,
			Email:          response_data.UserData.Email,
			Contact:        response_data.UserData.Contact,
			UserType:       response_data.UserData.UserType,
		}, err
	}
